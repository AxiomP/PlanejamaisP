...
export async function POST(request: Request) {
  try {
    ...
    if (!inputData.textoOriginal || inputData.textoOriginal.length < 50) {
      return NextResponse.json(
        { error: 'O texto original deve ter pelo menos 50 caracteres' },
        { status: 400 }
      )
    }

    if (inputData.textoOriginal.length > 5000) {
      return NextResponse.json(
        { error: 'O texto original deve ter no máximo 5000 caracteres' },
        { status: 400 }
      )
    }
    ...
    const { error: insertError } = await supabase.from('generations').insert({
      ...
      user_id: user.id,
      tool_type: TOOL_TYPE,
      title: `Reescrita: ${inputData.objetivo}`,
    ...
    console.error('Erro no endpoint reescritor:', error)
    return NextResponse.json(
